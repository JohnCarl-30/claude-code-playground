...
export function WorkspacePanel({
  ...
  const [tab, setTab] = useState<"run" | "changes" | "files">("run");
  ...
  useEffect(() => {
    ...
    setPrevious(
      last && last.template === workspace.template
        ? Object.fromEntries(last.files.map((f) => [f.path, f.content]))
        : {},
    );
  ...
  const current =
    ...
    files[0];
  const changed = (f: WorkspaceFile) =>
    f.path in previous && previous[f.path] !== f.content;
  const added = (f: WorkspaceFile) =>
      ...
      <header className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-2.5">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() =>
              window.confirm(
                "Reset this starter to its original files? Your changes to it will be lost (download a .zip first to keep them).",
              ) && onReset()
            }
            disabled={busy || !workspace}
            ...
            {files.map((f) => (
              <li key={f.path} className="shrink-0">
                ...
                >
                  ...
                  {added(f) && (
                    <span className="rounded bg-ok-soft px-1 text-[10px] text-ok">
                      new
                    </span>
                  )}
                </button>
