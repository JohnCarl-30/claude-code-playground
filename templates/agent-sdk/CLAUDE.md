...
- `guards.mjs` holds guardrails for the agent: a PreToolUse hook (`blockDangerousCommands`), a `canUseTool` callback and a `reviewer` subagent definition. Keep those export names; the playground's checks call them. Wire them into `query()` options as `hooks: { PreToolUse: [{ matcher: "Bash", hooks: [...] }] }`, `canUseTool` and `agents: { reviewer }`.
