...
- **Study plan:** enter your exam date and how much time you have each day, and get a day-by-day checklist: a baseline mock exam, then your weakest, heaviest domains first, with mistakes-deck rounds every few days and a mock exam each week. Tasks tick themselves off as you pass challenges, try examples, pass quizzes and take mock exams; **Rebuild from today** replans if you fall behind.
...
- **Exam-style questions:** candidates report the real exam has no code at all: scenario judgment ("how would you guide the team?"), model tiers instead of model names, and often a question of *what comes first*. Each question is tagged **judgment** (like the exam) or **detail** (names, numbers, syntax: good for depth, but reportedly not on the exam). The mock exam uses judgment questions only; knowledge checks prefer them, with a switch to include detail ones.
- **Mock exam:** a timed practice run shaped like the real one: 53 questions in 120 minutes from a bank of 280+, drawn from each domain by its weight, one question per screen with flag-for-review and a question grid. Results show an estimated score on the 100–1,000 scale, your percent correct by domain (like the real score report), and a review of every question with its explanation and source. **Retry the ones I missed** drills just those. Your place and the clock survive a reload.
...
The questions are written for this playground and checked against the official docs (for general software-engineering topics such as requirement types, life-cycle phases, CI and code review, GitHub's docs and Wikipedia serve as general references); they are not questions from the real exam. Every question carries the exact quote from the docs that backs its answer, and `npm run verify:quizzes` re-checks every source page and quote against the live docs. A GitHub Action runs that check every Monday and opens an issue when something changed, so questions that go out of date get caught. Answer length is balanced so the longest option isn't a giveaway (a test enforces it). Weight tells you where to spend time: Applications and Integration (33.1%) and Model Selection and Optimization (16.8%) are half the exam, so the Claude API challenges matter more than the Claude Code ones (3.1%).
